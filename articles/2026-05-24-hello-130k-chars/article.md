...
我昨天用工具去看了一下。在装了一些 skill、一些工具之后，第一个 request 过去，是**13 万字**。
...
有系统的 prompt，安全的、各个 section 的，反正一大坨。
有你所有的工具，它缺省就装了四十几个工具，还在不断加。
有你装的所有 skill，假设有 100 个 skill，每一个 skill 的 header 全都要编进去。
有你前面所有的上下文，所有工具处理的结果。
...
如果按输入 token 数来算，从 23 年到现在，我们每一次跟大语言模型的交互，已经提升了**几千倍**。
...
所以，所谓**智能消耗的爆炸**，不是一个未来的预言，它现在已经显现了。
