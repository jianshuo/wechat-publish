...
我这几个月给自己做的东西，全是这一类，随手数一数就有一把：

- cchour：一行命令，把 Claude Code 和 Codex 一天的日志拢成一张好看的报表，告诉我今天在 AI 上花了多长时间；
- cclight：Claude Code 在想事情的时候，MacBook 的刘海会亮起来，不用开着窗口盯，也知道他在干活；
- ccline：在命令行里直接问一句话，它把该敲的命令准备好，一个回车就能跑；
- ccglass：实时看我的 agent 到底给大模型发了什么、花了多少 token；
- cathier：一个 iPhone 上的小应用，扫一遍身体、给当下的情绪起个名字，再让 AI 陪我回头看看；
- 还有一个发公众号的工具，和给 jianshuo.dev 配的一堆小页面。
